...
void GpuBuffer::create(const std::wstring &name, uint32_t numElements, uint32_t elementSize, const UploadBuffer &uploadBuffer)
{
  ...
  auto newState = D3D12_RESOURCE_STATE_COPY_DEST;
  auto copyBarrier = CD3DX12_RESOURCE_BARRIER::Transition(m_pResoucrce.Get(), m_currentState, newState);
  ...
  auto readBarrier = CD3DX12_RESOURCE_BARRIER::Transition(m_pResoucrce.Get(), newState, D3D12_RESOURCE_STATE_GENERIC_READ);
